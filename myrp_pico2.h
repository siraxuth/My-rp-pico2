...
float kd_f = 0.95, kd_b = 0.025;                               // Kd PID
...
int adjust_power = 25;      // กำลังที่ใช้หมุนปรับตำแหน่ง (เพิ่ม/ลดได้)
...
void setup_robot()
{
  ...
  tone(9, 3000, 100); // ติ๊ดเสียงแจ้งว่าพร้อม
  get_EEP_Program();
  ...
  if (!my.begin())
  {
    // หากเริ่มต้น Gyro ไม่สำเร็จ ให้ beep เตือน
    tone(9, 1000, 200);
    delay(200);
  }
...
void read_eepC()
{
  ...
  Serial.print("Sensor C0 (Pin A0) => Max: ");
  Serial.print(readMaxC[0]);
  Serial.print(", Min: ");
  Serial.println(readMinC[0]);
  Serial.print("Sensor C1 (Pin A1) => Max: ");
...
void rotate_right(int speed, int degree, int offset)
{
  ...
  unsigned long timeout = max(2000, (unsigned long)(abs(degree) * 15)); // เพิ่ม timeout
  ...
  while (true)
  {
    ...
    {
      integral += error * dt;
      integral = constrain(integral, -50, 50); // จำกัด integral windup
...
void fw_gyro(int spl, int spr, float kp, float distance, int offset)
{
  ...
  float speed_scale = 0.70; // ⭐ ใส่ค่าที่แก้แล้ว
  ...
  // ========== Phase 2: ปรับหัวให้ตรง (Yaw = 0) ==========
  delay(100); // รอให้หุ่นหยุดนิ่ง

  _integral = 0;
  _prevErr = 0;
  prevT = millis();

  unsigned long correction_start = millis();
  float correction_kp = 0.8; // ⭐ ค่า Kp สำหรับปรับหัว (อาจต้องปรับ)
  float tolerance = 0.5;     // ⭐ ยอมรับความคลาดเคลื่อน ±0.5 องศา

  while (millis() - correction_start < 2000) // timeout 2 วินาที
  {
    unsigned long now = millis();
    float dt = (now - prevT) / 1000.0;
    if (dt <= 0)
    ...
    float corr = correction_kp * err + 0.0005 * _integral + 0.1 * deriv;

    // หมุนซ้าย/ขวา เพื่อแก้มุม
    int turnSpeed = constrain(corr, -40, 40);
    Motor(-turnSpeed, turnSpeed); // หมุนซ้าย/ขวา
  ...
  if (sp == 'p')
  {
    ...
    else if (sensorss == "c0")
    {
      do
      {
        Motor(spl, spr);
        delayMicroseconds(50);
      } while (analogRead(26) < (sensorMinC[0] + sensorMaxC[0]) / 2);
    }
    else if (sensorss == "c1")
    {
      do
      {
        Motor(spl, spr);
        delayMicroseconds(50);
      } while (analogRead(27) < (sensorMinC[0] + sensorMaxC[1]) / 2);
    }
  }
  else
  {
...
void bw_gyro(int spl, int spr, float kp, float distance, int offset)
{
  ...
  float speed_scale = 0.68; // ⭐ ค่าสำหรับถอยหลัง
  ...
  // ========== เดินถอยหลังพร้อมรักษามุม Yaw = 0 ==========
  while (1)
  {
    unsigned long now = millis();
    float dt = (now - prevT) / 1000.0;
    ...
    float yaw = my.gyro('z') - yaw_offset;
    float err = yaw; // ต้องการให้ yaw = 0 ตลอด
    ...
    // ⭐ สำคัญ: ถอยหลัง = ความเร็วติดลบ
    // แต่การแก้มุมยังคงเหมือนเดินหน้า (เพราะอ้างอิงจาก gyro)
    int leftSpeed = constrain(-spl - corr, -100, 100);  // ติดลบ = ถอยหลัง
    int rightSpeed = constrain(-spr + corr, -100, 100); // ติดลบ = ถอยหลัง
  ...
  if (offset > 0)
  {
    Motor(15, 15); // ⭐ เดินหน้าเบรก (ตรงข้ามกับถอยหลัง)
    delay(offset);
    Motor(1, 1);
    delay(10);
  }
  ...
  unsigned long correction_start = millis();
  float correction_kp = 0.8;
  float tolerance = 0.5;

  while (millis() - correction_start < 2000)
  {
    unsigned long now = millis();
    float dt = (now - prevT) / 1000.0;
    if (dt <= 0)
...
void bw_gyro(int spl, int spr, float kp, int distance, String sensorss, char sp, int offset)
{
  ...
  float speed_scale = 0.68; // ใช้ค่าที่คาลิเบรตจาก fw()
  ...
  while (1)
  {
    ...
    else if (sensorss == "c01" || sensorss  == "c10")
    {
      if (analogRead(A0) < (sensorMinC[0] + sensorMaxC[0]) / 2 &&
          analogRead(A1) < (sensorMinC[1] + sensorMaxC[1]) / 2)
      {
        break;
      }
    }
...
// ปรับ position_4sensor ให้ดูดกลับเข้าหาเส้นแรงขึ้นตอนหลุด
int position_4sensor()
{
  int Minsensor_values_A[] = {sensorMinA[2], sensorMinA[3], sensorMinA[4], sensorMinA[5]};
  int Maxsensor_values_A[] = {sensorMaxA[2], sensorMaxA[3], sensorMaxA[4], sensorMaxA[5]};
  ...
  {
    long value = map(read_sensorA(sensor_pin_4[i]),
    Minsensor_values_A[i],
    Maxsensor_values_A[i], 1000, 0);
  ...
  // ตอนหลุดเส้น ให้คืนค่าที่แรงขึ้น เพื่อให้หมุนกลับเข้าหาเส้นเอง
  if (!onLine)
  {
    if (_lastPosition < (numSensor4 - 1) * 1000 / 2)
    {
      return -2000; // หลุดซ้าย → คืนค่าติดลบแรงๆ เพื่อหมุนกลับ
    }
    else
    {
      return 5000; // หลุดขวา → คืนค่าบวกแรงๆ เพื่อหมุนกลับ
    }
  }
...
// ปรับ error_4sensor ให้คืนค่า error ที่เหมาะสมกับ kp ต่ำๆ float 
float error_4sensor()
{
  ...
  // ปรับสเกล error ให้เหมาะสม (ลองปรับตัวเลขนี้)
  // ถ้า error ยังแรงเกินไป ลองเปลี่ยนเป็น /5 หรือ /8
  errors = errors / 3.0;
